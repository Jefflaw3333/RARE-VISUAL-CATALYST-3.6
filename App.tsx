...
const FocusableImage: React.FC<FocusableImageProps> = ({ src, focusArea, onChange }) => {
    ...
    const handleMouseDown = (e: MouseEvent<HTMLImageElement>) => {
        ...
        setDragRect({ x: coords.x, y: coords.y, width: 0, height: 0 });
    };
    ...
    useEffect(() => {
        const handleMouseMove = (e: globalThis.MouseEvent) => {
            ...
            const x = Math.max(0, Math.min(selectionStart.x, currentX));
            const y = Math.max(0, Math.min(selectionStart.y, currentY));
            const width = Math.min(Math.abs(currentX - selectionStart.x), maxX - x);
            const height = Math.min(Math.abs(currentY - selectionStart.y), maxY - y);
...
const App: React.FC = () => {
    ...
    // Check for API Key on mount — use apiKeyStore
    useEffect(() => {
            ...
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden font-sans">
                ...
                <div className="bg-slate-900 border border-slate-700 p-10 rounded-2xl max-w-md text-center shadow-2xl relative z-10 animate-fade-in">
                    ...
                    <p className="text-slate-400 mb-8 leading-relaxed">
                        ...
                        <br /><br />
                        请先点击下方按钮配置你的 API Key 和自定义端点。
                    </p>
                    <button
                        onClick={handleConnectApiKey}
                        className="w-full py-3.5 bg-lime-400 hover:bg-lime-300 text-slate-900 font-bold rounded-xl transition-colors text-lg shadow-lg"
                    >
                        ⚙️ 配置 API Key
                    </button>
                    <p className="text-xs text-slate-600 mt-6">
                        支持自定义 Gemini 代理端点和模型名称
                    </p>
                </div>
                <ApiKeyModal isOpen={showApiKeyModal} onClose={() => { setShowApiKeyModal(false); setHasApiKey(!!getGeminiApiKey()); }} />
            </div>
        );
    }
        ...
        <div className="min-h-screen main-container text-slate-200 font-sans relative">
            ...
            <main className="container mx-auto px-4 py-8 relative z-10">
                <header className="text-center mb-12 relative">
                    {/* UPDATED HEADER with Visual Effects + Settings Button */}
                    ...
                    <button
                        ...
                        className="absolute top-1 right-0 p-2 text-slate-500 hover:text-lime-400 transition-colors rounded-lg hover:bg-slate-800/50"
                        title="API 设置"
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className="w-full mt-6 py-4 bg-gradient-to-r from-lime-500 to-emerald-500 hover:from-lime-400 hover:to-emerald-400 text-black font-bold text-lg rounded-xl shadow-lg shadow-lime-900/20 transition-all hover:scale-[1.02] disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-2"
                        >
                            {isLoading ? (
                                <>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        {(isLoading || isBrainstorming || isAnalyzing) && (
