...
const App: React.FC = () => {
    ...
    const [outfitReferenceImages, setOutfitReferenceImages] = useState<string[]>([]);
    ...
    const handleFilesSelected = (files: FileList | null) => {
        if (!files) return;
        const filesToProcess = Array.from(files).slice(0, 6 - uploadedImages.length);
    ...
    const handleOutfitReferenceFilesSelected = (files: FileList | null) => {
        if (!files) return;
        const filesToProcess = Array.from(files).slice(0, 3 - outfitReferenceImages.length);

        filesToProcess.forEach(file => {
            const reader = new FileReader();
            reader.onloadend = () => {
                setOutfitReferenceImages(prev => [...prev, reader.result as string]);
            };
            reader.readAsDataURL(file);
        });
    };

    const handleRemoveOutfitReferenceImage = (indexToRemove: number) => {
        setOutfitReferenceImages(prev => prev.filter((_, index) => index !== indexToRemove));
    };
    ...
    const handleLoadPreset = (presetName: string) => {
        ...
        if (preset) {
            ...
            setOutfitReferenceImages(settings.outfitReferenceImages || []);
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            const outfitReferenceImagesRef = outfitReferenceImages.map(img => ({
                base64: img.split(',')[1],
                mimeType: img.split(';')[0].split(':')[1],
            }));
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    <div className="lg:col-span-4 space-y-6">
                        <div className="bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 p-6 rounded-xl shadow-lg">
                            ...
                            {uploadedImages.length === 0 ? (
                                ...
                                >
                                    <div className="text-center">
                                        ...
                                        <span className="block text-xs">Up to 6 images (最多6张)</span>
                                    </div>
                                </button>
                            ) : (
                            ...
                            <div className="mt-6 border-t border-slate-700 pt-4">
                                <h3 className="text-base font-semibold text-slate-300 mb-3">Clothes / Outfit Reference (衣服/穿搭参考)</h3>
                                <input
                                    type="file"
                                    multiple
                                    accept="image/*"
                                    ref={outfitFileInputRef}
                                    onChange={(e) => handleOutfitReferenceFilesSelected(e.target.files)}
                                    className="hidden"
                                />
                                <div className="grid grid-cols-3 gap-3">
                                    {outfitReferenceImages.map((image, index) => (
                                        <div key={index} className="relative aspect-square group col-span-1">
                                            <img src={image} alt={`Outfit Reference ${index + 1}`} className="w-full h-full object-cover rounded-md shadow-md" />
                                            <button onClick={() => handleRemoveOutfitReferenceImage(index)} className="absolute top-1 right-1 p-1 bg-slate-900/70 rounded-full text-slate-300 hover:bg-red-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100 z-10">
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                            </button>
                                        </div>
                                    ))}
                                    {outfitReferenceImages.length < 3 && (
                                        <button
                                            onClick={
                                        ...
                                        >
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
                                            <span className="text-xs mt-1">Add Outfit Ref</span>
                                        </button>
                                    )}
                                </div>
                                <p className="text-xs text-slate-500 mt-2">Upload up to 3 images to guide the character's clothing and style.</p>
                            </div>
                                            ...
                                            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                                                {socialPlatforms.map(item => (
                                                    <button
                                                        ...
                                                        className={`flex flex-col items-center justify-center gap-1 p-2 rounded-lg border transition-all duration-200 ${selectedSocialPlatforms.includes(item.id)
                                                                ? 'bg-lime-900/50 border-lime-400 text-lime-300 ring-2 ring-lime-400/50'
